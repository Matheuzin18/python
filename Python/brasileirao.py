...
def gerar_relatorio_brasileirao (nome_arquivo, ano):
    ...
    '''
    Utiliza as funções anteriores para gerar um relatório com as estatisticas do Campeonato Brasileiro de futebol do ano indicado pelo usuário.
    ...
    jogador_mais_vitorias = time_com_mais_vitorias(dados)
    media_gols = calcular_media_gols(dados)
    partida_maior_gols = partida_com_maior_numero_de_gols(dados)
    rodada_maior_gols = partida_maior_gols['rodada']
    mandante_maior_gols = partida_maior_gols['mandante']
    gols_mandante_maior_gols = partida_maior_gols['mandante_gols']
    visitante_maior_gols = partida_maior_gols['visitante']
    gols_visitante_maior_gols = partida_maior_gols['visitante_gols']
    vencedor_maior_gols = partida_maior_gols['vencedor']

    relatorio = f'Relatório do Campeonato Brasileiro de {ano}:\n'
    relatorio += '--------------------\n'
    relatorio += f'Total de partidas: {total_partidas}\n'
    relatorio += f'Time com mais vitórias: {jogador_mais_vitorias}\n'
    relatorio += f'Média de gols por partida: {media_gols}\n'
    relatorio += 'Partida com maior número de gols:\n'
